...
public class MainControl : MonoBehaviour {
		...
		for (int z=0;z<this.webJSONAllTransactions.Count;z++) {
            ...
            if (node != null)
            {
                ...
                RepeaterItemValue value1 = new RepeaterItemValue()
                {
                    ...
                    PropertyType = RepeaterItemValue.PropertyTypes.DateTime,
                    PropertyValue = node[3].Value ?? new System.DateTime(1900, 1, 1).ToShortDateString()
                };
                RepeaterItemValue value2 = new RepeaterItemValue()
                {
                    PropertyName = "BusinessName",
                    PropertyType = RepeaterItemValue.PropertyTypes.String,
                    PropertyValue = node[6].Value ?? ""
                };
                RepeaterItemValue value3 = new RepeaterItemValue()
                {
                    PropertyName = "ItemName",
                    PropertyType = RepeaterItemValue.PropertyTypes.String,
                    PropertyValue = node[7].Value ?? ""
                };
                RepeaterItemValue value4 = new RepeaterItemValue()
                {
                    PropertyName = "ItemAmount",
                    PropertyType = RepeaterItemValue.PropertyTypes.Float,
                    PropertyValue = node[8].Value ?? ""
                };
		...
		for (int z=0;z<this.webJSONPMName.Count;z++) {
            ...
            if (node != null)
            {
                ...
                RepeaterItemValue value1 = new RepeaterItemValue()
                {
                    PropertyName = "ItemName",
                    PropertyType = RepeaterItemValue.PropertyTypes.String,
                    PropertyValue = node[1].Value
                };
                RepeaterItemValue value2 = new RepeaterItemValue()
                {
                    PropertyName = "LowestBusiness",
                    PropertyType = RepeaterItemValue.PropertyTypes.String,
                    PropertyValue = node[3].Value
                };
                RepeaterItemValue value3 = new RepeaterItemValue()
                {
                    PropertyName = "LowestPrice",
                    PropertyType = RepeaterItemValue.PropertyTypes.Float,
                    PropertyValue = node[4].Value
                };
                RepeaterItemValue value4 = new RepeaterItemValue()
                {
                    PropertyName = "HighestBusiness",
                    PropertyType = RepeaterItemValue.PropertyTypes.String,
                    PropertyValue = node[5].Value
                };
	...
	void Update () {
		...
		if (SecondsBeforeMeltdown > 0f) {
			...
			foreach(var value in barChart.ChartData.GetChartData()) {
				value.Value = Convert.ToDecimal(UnityEngine.Random.Range(0f,60f));
			}
			barChart.DrawChart();
			...
			foreach(var value in pieChart.ChartValues.GetChartData()) {
				value.Value = Convert.ToDecimal(UnityEngine.Random.Range(10f,60f));
			}
			pieChart.CalculatePieChartValues();
			pieChart.DrawPie();
